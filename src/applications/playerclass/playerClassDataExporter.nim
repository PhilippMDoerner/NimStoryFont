...
proc exportPlayerClassData*(con: DbConn, campaign: CampaignRead): JsonNode =
  result = newJObject()
  let campaignPlayerClasses = con.getCampaignPlayerClasses(ReadListParams(campaignName: campaign.name))
  let serializedCampaignPlayerClasses = con.serializePlayerClasses(campaignPlayerClasses)
