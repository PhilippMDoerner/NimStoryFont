...
proc exportSessionData*(con: DbConn, campaign: CampaignRead): JsonNode =
  result = newJObject()
  let campaignSessions = con.getCampaignSessions(ReadListParams(campaignName: campaign.name))
  ...
  for index, session in campaignSessions:
    ...
    let nameUrl = fmt"/session/{campaign.name}/{session.sessionNumber}/{session.isMainSession.int}/"
