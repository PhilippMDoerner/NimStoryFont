...
proc exportDiaryentryData*(con: DbConn, campaign: CampaignRead): JsonNode =
  result = newJObject()
  let campaignDiaryentrys = con.getCampaignDiaryEntries(ReadListParams(campaignName: campaign.name))
  ...
  for diaryentry in campaignDiaryentrys:
    let url = fmt"/diaryentry/{campaign.name}/{diaryentry.sessionId.sessionNumber}/{diaryentry.sessionId.isMainSession.int}/{diaryentry.authorId.username}/"
