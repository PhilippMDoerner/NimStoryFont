...
// src/viewport/resolveViewport.ts
var VIEWPORT_MIN_WIDTH = 40, VIEWPORT_MIN_HEIGHT = 40, URL_VALUE_PATTERN = /^([0-9]{1,4})([a-z]{0,4})-([0-9]{1,4})([a-z]{0,4})$/, normalizeGlobal = (value, defaultIsRotated) => typeof value == "string" ? { value, isRotated: defaultIsRotated } : { value: value?.value, isRotated: value?.isRotated ?? defaultIsRotated }, resolveViewport = ({
  globals,
  storyGlobals,
  userGlobals,
  options,
  ...
  let global = normalizeGlobal(globals?.[PARAM_KEY]), userGlobal = normalizeGlobal(userGlobals?.[PARAM_KEY]), storyGlobal = normalizeGlobal(storyGlobals?.[PARAM_KEY]), storyHasViewport = PARAM_KEY in storyGlobals, primaryGlobal = storyHasViewport ? storyGlobal : userGlobal, secondaryGlobal = storyHasViewport ? userGlobal : storyGlobal, value = primaryGlobal?.value ?? secondaryGlobal?.value ?? global?.value, isRotated = primaryGlobal?.isRotated ?? secondaryGlobal?.isRotated ?? global?.isRotated ?? !1, keys = Object.keys(options), isLocked = disable || PARAM_KEY in storyGlobals || !keys.length, [match, vx, ux, vy, uy] = value?.match(URL_VALUE_PATTERN) || [];
  if (match) {
    let x = ux && ux !== "px" ? vx : Math.max(Number(vx), VIEWPORT_MIN_WIDTH), y = uy && uy !== "px" ? vy : Math.max(Number(vy), VIEWPORT_MIN_HEIGHT), width = `${x}${ux === "pct" ? "%" : ux || "px"}`, height = `${y}${uy === "pct" ? "%" : uy || "px"}`, selection = lastSelectedOption ? options[lastSelectedOption] : void 0;
    return {
  ...
  if (value && keys.length) {
    let { name, styles, type = "other" } = options[value] ?? options[keys[0]];
    return {
      name,
      type,
...
}, toViewportPixels = (dimension, reference) => {
  let pxMatch = dimension.match(/^(\d+(?:\.\d+)?)px$/);
  if (pxMatch)
    return Math.round(Number(pxMatch[1]));
  let percentMatch = dimension.match(/^(\d+(?:\.\d+)?)%$/);
  if (percentMatch)
    return Math.round(reference * Number(percentMatch[1]) / 100);
}, toResolvedViewportDimensions = (viewport, reference) => ({
  name: viewport.name,
