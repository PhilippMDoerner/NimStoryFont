...
var customHeadHasFavicon = (head) => (head.match(/<link\b[^>]*>/gi) || []).some((tag) => {
  let rel = tag.match(/\brel\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s>]+))/i);
  return (rel?.[1] || rel?.[2] || rel?.[3] || "").split(/\s+/).some((token) => token.toLowerCase() === "icon");
}), renderHTML = async (template, title, favicon, customHead, cssFiles, jsFiles, features, refs, logLevel, docsOptions, tagsOptions, { versionCheck, previewUrl, configType, ignorePreview }, globals) => {
  let titleRef = await title, templateRef = await template, headRef = await customHead || "", stringifiedGlobals = Object.entries(globals).reduce(
...
var buildFrameworkGlobalsFromOptions = async (options) => {
  let globals = {}, { builder: builderConfig, channelOptions } = await options.presets.apply("core"), builderName = typeof builderConfig == "string" ? builderConfig : builderConfig?.name, builder2 = Object.values(SupportedBuilder).find((builder3) => builderName?.includes(builder3)), frameworkName = await getFrameworkName(options), frameworkPackageName = extractFrameworkPackageName(frameworkName), framework = frameworkPackages[frameworkPackageName], renderer = frameworkToRenderer[framework];
  return options.configType === "DEVELOPMENT" && (globals.CHANNEL_OPTIONS = { wsToken: channelOptions?.wsToken }), globals.STORYBOOK_BUILDER = builder2, globals.STORYBOOK_FRAMEWORK = framework, globals.STORYBOOK_RENDERER = renderer, globals.STORYBOOK_NETWORK_ADDRESS = options.networkAddress, globals;
};
