...
var require_common = __commonJS({
  "../../node_modules/comment-json/src/common.js"(exports, module) {
    var {
      ...
      PREFIX_BEFORE_ALL,
      PREFIX_AFTER_ALL
    ].map(Symbol.for), COLON = ":", UNDEFINED = void 0, symbol = (prefix, key) => /* @__PURE__ */ Symbol.for(prefix + COLON + key), define2 = (target, key, value) => Object.defineProperty(target, key, {
      value,
      writable: !0,
      configurable: !0
...
`, STR_NULL = "null", BEFORE = (prop) => `${PREFIX_BEFORE}:${prop}`, AFTER_PROP = (prop) => `${PREFIX_AFTER_PROP}:${prop}`, AFTER_COLON = (prop) => `${PREFIX_AFTER_COLON}:${prop}`, AFTER_VALUE = (prop) => `${PREFIX_AFTER_VALUE}:${prop}`, AFTER = (prop) => `${PREFIX_AFTER}:${prop}`, meta = {
    ...
    }, join4 = (one, two, gap) => one ? two ? one + two.trim() + LF + gap : one.trimRight() + LF + gap : two ? two.trimRight() + LF + gap : EMPTY, join_content = (inside, value, gap) => {
      ...
      return keys.forEach(iteratee), inside += join4(
        after_comma,
        process_comments(value, PREFIX_AFTER, deeper_gap),
        deeper_gap
...
var ANGULAR_JSON_PATH = "angular.json", JSON_EDIT_FORMATTING = { insertSpaces: !0, tabSize: 2, eol: `
` }, editJsonText = (text, path2, value) => applyEdits(text, modify(text, path2, value, { formattingOptions: JSON_EDIT_FORMATTING })), isStorybookTarget = (target) => {
  if (typeof target != "object" || target === null)
    return !1;
  let ref = target.builder ?? target.executor;
  return typeof ref == "string" && (ref.endsWith(":start-storybook") || ref.endsWith(":build-storybook"));
}, AngularJSON = class {
  constructor(path2 = ANGULAR_JSON_PATH) {
    if (!existsSync2(path2))
      throw new MissingAngularJsonError({ path: resolve3(path2) });
    this.path = path2, this.rawText = readFileSync2(path2, "utf8"), this.json = JSON.parse(this.rawText);
  }
  /** Apply a format-preserving edit at `path` and keep `json` in sync with the result. */
  edit(path2, value) {
    this.rawText = editJsonText(this.rawText, path2, value), this.json = JSON.parse(this.rawText);
  }
  get projects() {
  ...
  get hasStorybookBuilder() {
    return Object.keys(this.projects).some((projectName) => {
      let { architect } = this.projects[projectName];
      return Object.keys(architect).some((key) => architect[key].builder === "@storybook/angular:start-storybook" || architect[key].builder === "@storybook/angular-vite:start-storybook");
    });
  }
  get rootProject() {
  ...
  addStorybookEntries({
    ...
    let { architect } = this.projects[angularProjectName], builderPackage = useVite ? "@storybook/angular-vite" : "@storybook/angular", baseOptions = {
    ...
    architect.storybook || this.edit(["projects", angularProjectName, "architect", "storybook"], {
      builder: `${builderPackage}:start-storybook`,
      options: {
        ...baseOptions,
        port: 6006
      }
    }), architect["build-storybook"] || this.edit(["projects", angularProjectName, "architect", "build-storybook"], {
...
var configureFlatConfig = async (code) => {
  let ast = babelParse(code), alreadyHasStorybookImport = !1;
  if (traverse(ast, {
    ImportDeclaration(path2) {
      path2.node.source.value === "eslint-plugin-storybook" && (alreadyHasStorybookImport = !0, path2.stop());
    },
    CallExpression(path2) {
      types.isImport(path2.node.callee) && path2.node.arguments.length > 0 && types.isStringLiteral(path2.node.arguments[0]) && path2.node.arguments[0].value === "eslint-plugin-storybook" && (alreadyHasStorybookImport = !0, path2.stop());
    }
  }), alreadyHasStorybookImport)
    return code;
  let tsEslintLocalName = "", eslintDefineConfigLocalName = "", eslintConfigExpression = null;
  return traverse(ast, {
    ImportDeclaration(path2) {
      ...
      if (path2.node.source.value === "eslint/config") {
        let defineConfigSpecifier = path2.node.specifiers.find(
          (s) => types.isImportSpecifier(s) && types.isIdentifier(s.imported, { name: "defineConfig" })
        );
        defineConfigSpecifier && types.isImportSpecifier(defineConfigSpecifier) && (eslintDefineConfigLocalName = defineConfigSpecifier.local.name);
      }
    },
    ExportDefaultDeclaration(path2) {
      ...
      if (types.isArrayExpression(eslintConfigExpression) && eslintConfigExpression.elements.push(types.spreadElement(storybookConfig)), types.isCallExpression(eslintConfigExpression) && types.isMemberExpression(eslintConfigExpression.callee) && tsEslintLocalName && types.isIdentifier(eslintConfigExpression.callee.object, { name: tsEslintLocalName }) && types.isIdentifier(eslintConfigExpression.callee.property, { name: "config" }) && eslintConfigExpression.arguments.push(storybookConfig), types.isCallExpression(eslintConfigExpression) && types.isIdentifier(eslintConfigExpression.callee) && eslintDefineConfigLocalName && eslintConfigExpression.callee.name === eslintDefineConfigLocalName && eslintConfigExpression.arguments.length > 0) {
        let firstArg = eslintConfigExpression.arguments[0];
        if (types.isExpression(firstArg)) {
          let unwrappedArg = unwrapTSExpression(firstArg);
          unwrappedArg && types.isArrayExpression(unwrappedArg) && unwrappedArg.elements.push(types.spreadElement(storybookConfig));
        }
      }
      if (types.isIdentifier(eslintConfigExpression)) {
        ...
        if (binding && types.isVariableDeclarator(binding.path.node)) {
          ...
          if (types.isArrayExpression(init))
            init.elements.push(types.spreadElement(storybookConfig));
          else if (types.isCallExpression(init) && init.arguments.length > 0 && types.isIdentifier(init.callee) && eslintDefineConfigLocalName && init.callee.name === eslintDefineConfigLocalName) {
...
async function configureEslintPlugin({
  ...
  if (eslintConfigFile)
    ...
    let { packageJson, operationDir } = packageManager.primaryPackageJson, existingExtends = normalizeExtends(packageJson.eslintConfig?.extends).filter(Boolean);
...
// src/cli/AddonVitestService.ts
var AddonVitestService = class _AddonVitestService {
   ...
   */
  static getComparableVersion(specifier) {
    if (!specifier)
      return;
    let range = validRange(specifier);
    return coerce2(range ? minVersion(range)?.version : specifier)?.version;
  }
  /**
  ...
  async collectDependencies() {
    let allDeps = this.packageManager.getAllDependencies(), dependencies = [], vitestVersionSpecifier = await this.packageManager.getDeclaredVersionSpecifier("vitest"), versionToCheck = _AddonVitestService.getComparableVersion(vitestVersionSpecifier), basePackages = [
      "vitest",
      "playwright",
      (versionToCheck ? satisfies2(versionToCheck, ">=4.0.0") : !0) ? "@vitest/browser-playwright" : "@vitest/browser"
    ];
    ...
    if (!v8Version && !istanbulVersion && dependencies.push("@vitest/coverage-v8"), !vitestVersionSpecifier)
      return dependencies;
    let related = dependencies.filter((pkg) => pkg.includes("vitest")), rest = dependencies.filter((pkg) => !pkg.includes("vitest"));
    return [
  ...
  async installPlaywright(options = {}) {
    let errors = [], platform = os.platform(), useWithDeps = !!process.env.CI || platform === "darwin" || platform === "win32", playwrightCommand = useWithDeps ? ["playwright", "install", "chromium", "--with-deps"] : ["playwright", "install", "chromium"], playwrightCommandString = this.packageManager.getPackageCommand(playwrightCommand), result;
    if (process.env.STORYBOOK_CLI_SKIP_PLAYWRIGHT_INSTALLATION)
      return result = "skipped", { errors, result };
    try {
      (options.yes ? !0 : await (async () => (logger3.log(import_ts_dedent2.dedent`
        ...
        }),
        {
          ...
          abortable: !0
      ...
      ) ? result = "aborted" : (result = "installed", useWithDeps || logger3.warn(import_ts_dedent2.dedent`
            ...
            `)) : (logger3.warn("Playwright installation skipped"), result = "skipped");
    } catch (e) {
  ...
  async validatePackageVersions() {
    let reasons = [], vitestVersionSpecifier = await this.packageManager.getInstalledVersion("vitest"), coercedVitestVersion = vitestVersionSpecifier ? coerce2(vitestVersionSpecifier) : null, isCanary = coercedVitestVersion?.version.startsWith("0.0.0") ?? !1;
    coercedVitestVersion && !satisfies2(coercedVitestVersion, ">=3.0.0") && !isCanary && reasons.push(
  ...
  async validateConfigFiles(directory) {
    ...
    if (vitestWorkspaceFile?.endsWith(".json"))
      ...
      canUpdateVitestWorkspaceFile(fileContents) || reasons.push(`Found an invalid workspace config file: ${vitestWorkspaceFile}`);
    }
    ...
    if (vitestConfigFile?.endsWith(".cts") || vitestConfigFile?.endsWith(".cjs"))
      ...
      canUpdateVitestConfigFile(configContent) || reasons.push(`Found an invalid Vitest config file: ${vitestConfigFile}`);
    }
    return reasons.length > 0 ? { compatible: !1, reasons } : { compatible: !0 };
  }
};
