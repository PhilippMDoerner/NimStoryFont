...
import {
  instructions
} from "../_node-chunks/chunk-VLAMGE2J.js";
import "../_node-chunks/chunk-FDJYLTPT.js";
import "../_node-chunks/chunk-DPXVO6NW.js";
import {
  CLAUDE_AGENT_NAME,
...
import {
  detectLanguage,
  getStorybookData
} from "../_node-chunks/chunk-KD5NGLH7.js";
import {
  array,
  boolean,
  integer,
...
import { withTelemetry as withTelemetry5 } from "storybook/internal/core-server";
import { logTracker, logger as logger4 } from "storybook/internal/node-logger";
...
// src/cli/ai/setup-prompts/index.ts
var import_ts_dedent = __toESM(require_dist(), 1);

// src/cli/ai/utils/project-overview.ts
...
var DEFAULT_PROMPT_NAME = "optimized-tests", CURRENTLY_USED_PROMPT = {
  ...
  monorepo: async () => (await import("../_node-chunks/monorepo-7R4NQ2IG.js")).instructions,
  "optimized-tests": async () => (await import("../_node-chunks/optimized-tests-5J6VSNWX.js")).instructions,
  "relaxed-limits": async () => (await import("../_node-chunks/relaxed-limits-PUF3NMZ4.js")).instructions,
  setup: async () => (await import("../_node-chunks/setup-XCCD5YHG.js")).instructions,
  "pattern-copy-play": async () => (await import("../_node-chunks/pattern-copy-play-3IY6KXXL.js")).instructions,
  "monorepo-optimized-tests-relaxed-limits-no-story-deletion": async () => (await import("../_node-chunks/monorepo-optimized-tests-relaxed-limits-no-story-deletion-XW2MTZDH.js")).instructions
}, PROMPT_NAMES = [
...
// src/cli/ai/mcp/client.ts
import { versions } from "storybook/internal/common";

// src/cli/ai/mcp/types.ts
...
// src/cli/ai/mcp/client.ts
var STORYBOOK_MCP_PROXY_HEADER = "X-Storybook-MCP-Proxy", STORYBOOK_MCP_PROXY_HEADER_VALUE = "true", REQUEST_TIMEOUT_MS = 600 * 1e3, MCP_CLIENT_INFO = { name: "storybook-cli", version: versions.storybook }, MCP_PROTOCOL_VERSION = "2025-06-18", McpJsonRpcError = class extends Error {
...
async function sendJsonRpcRequest(record2, method, params, resultSchema, fetchImpl) {
  ...
  let target = new URL(endpoint, record2.url).href, sessionId = await initializeMcpSession(target, fetchImpl), response = await fetchImpl(target, {
    method: "POST",
    headers: {
      ...REQUEST_HEADERS,
...
${examples.join(`
`)}`;
}, buildPortMismatch = (port, records) => `Storybook is running for this project, but not on port \`${port ?? "unknown"}\`. Retry with one of the running ports below, or omit \`--port\` to route by project alone.
...
function resolveStorybookConfigDir({ cwd, configDir } = {}) {
  let projectCwd = resolve2(cwd ?? process.cwd());
  return configDir ? isAbsolute(configDir) ? configDir : resolve2(projectCwd, configDir) : resolve2(projectCwd, ".storybook");
}
...
// src/cli/ai/mcp/resolve-instance.ts
import { resolve as resolve3 } from "node:path";
function resolveInstance(records, target) {
  let { port: targetPort, agent: currentAgent } = target, normalisedCwd = resolve3(target.cwd), normalisedConfigDir = target.configDir && resolve3(target.configDir), matchesConfigDir = (r) => normalisedConfigDir != null && r.configDir != null && resolve3(r.configDir) === normalisedConfigDir, projectMatches = target.configDirExplicit ? records.filter(matchesConfigDir) : records.filter((r) => resolve3(r.cwd) === normalisedCwd || matchesConfigDir(r)), matches = targetPort == null ? projectMatches : projectMatches.filter((r) => r.port === targetPort);
  if (matches.length === 0)
    return targetPort != null && projectMatches.length > 0 ? {
      kind: "intercept",
  ...
  let sortedMatches = selectCompetingBucket(matches, targetPort, currentAgent), selected = sortedMatches.find((r) => r.mcp.status === "ready") ?? sortedMatches[0];
...
function selectCompetingBucket(matches, targetPort, currentAgent) {
  if (targetPort != null)
...
async function loadLocalMetadata(cwd, configDir, deps) {
  let resolvedCwd = resolve4(cwd ?? process.cwd()), resolvedConfigDir = resolveStorybookConfigDir({ cwd: resolvedCwd, configDir }), loadMetadata = deps.loadStorybookAiMetadata ?? loadStorybookAiMetadata;
  try {
    return {
      kind: "ok",
      cwd: resolvedCwd,
...
async function resolveReadyInstance(target, deps) {
  let cwd = resolve4(target.cwd ?? process.cwd()), configDir = resolveStorybookConfigDir({ cwd, configDir: target.configDir }), records = await readRegistry(deps.registryDir), resolution = resolveInstance(records, {
    cwd,
    configDir,
...
var CWD_DESCRIPTION = "Project directory of the target Storybook; place before the command name", CONFIG_DIR_DESCRIPTION = "Storybook config directory of the target Storybook; place before the command name", PORT_DESCRIPTION = "Port of the target Storybook for runtime commands; place before the command name";
...
import { buildStaticStandalone, withTelemetry as withTelemetry2 } from "storybook/internal/core-server";
...
import { buildIndexStandalone, withTelemetry as withTelemetry3 } from "storybook/internal/core-server";
var buildIndex = async (cliOptions) => {
  ...
  await withTelemetry3("index", { cliOptions, presetOptions }, () => buildIndexStandalone(options));
};

// src/cli/dev.ts
var import_ts_dedent2 = __toESM(require_dist(), 1);
import { cache as cache4 } from "storybook/internal/common";
import { buildDevStandalone, withTelemetry as withTelemetry4 } from "storybook/internal/core-server";
import { logger as logger3, instance as npmLog } from "storybook/internal/node-logger";
function printError(error) {
  npmLog.heading = "", error instanceof Error ? error.error ? logger3.error(error.error) : error.stats && error.stats.compilation.errors ? error.stats.compilation.errors.forEach((e) => logger3.log(e)) : logger3.error(error) : error.compilation?.errors && error.compilation.errors.forEach((e) => logger3.log(e)), logger3.warn(
...
var handleCommandFailure = async (logFilePath) => {
  try {
    ...
    logger4.log(`Debug logs are written to: ${logFile}`);
  } catch {
  }
  logger4.outro("Storybook exited with an error"), process.exit(1);
  ...
  try {
    ...
    logger4.setLogLevel(loglevel), options.logfile && logTracker.enableLogWriting(), await globalSettings();
  } catch (e) {
    logger4.error(`Error loading global settings:
` + String(e));
  }
  ...
  if (logTracker.shouldWriteLogsToFile)
    try {
      ...
      logger4.outro(`Debug logs are written to: ${logFile}`);
    } catch {
    }
...
command("dev").option("-p, --port <number>", "Port to run Storybook").option("-h, --host <string>", "Host to run Storybook").option("-c, --config-dir <dir-name>", "Directory where to load Storybook configurations from").option(
  ...
  logger4.intro(`${packageJson.name} v${packageJson.version}`);
  let resolvedOptions;
  try {
    resolvedOptions = resolveDevCommandOptions(options);
  } catch (error) {
    return logger4.error(error instanceof Error ? error.message : String(error)), handleCommandFailure(options.logfile);
  }
...
command("build").option("-o, --output-dir <dir-name>", "Directory where to store built files").option("-c, --config-dir <dir-name>", "Directory where to load Storybook configurations from").option("--quiet", "Suppress verbose build output").option("--debug-webpack", "Display final webpack configurations for debugging purposes").option(
  ...
  let { default: packageJson } = await import("storybook/package.json", { with: { type: "json" } });
  logger4.intro(`Building ${packageJson.name} v${packageJson.version}`), getEnvConfig(options, {
    ...
    logger4.outro("Storybook exited with an error"), process.exit(1);
  }), logger4.outro("Storybook build completed successfully");
});
command("index").option("-o, --output-file <file-name>", "JSON file to output index").option("-c, --config-dir <dir-name>", "Directory where to load Storybook configurations from").option("--quiet", "Suppress verbose build output").action(async (options) => {
  ...
  env.NODE_ENV = env.NODE_ENV || "production";
  let { default: packageJson } = await import("storybook/package.json", { with: { type: "json" } });
  logger4.log(import_picocolors.default.bold(`${packageJson.name} v${packageJson.version}
...
var handleAiCommandFailure = (logFilePath) => async (error) => (error instanceof HandledError2 || logger4.error(String(error)), handleCommandFailure(logFilePath ?? !1)), aiCommand = command("ai").description("AI agent helpers for Storybook").option(
  "-o, --output <path>",
...
aiCommand.command("setup").description("Generate setup instructions to write stories for real components").addOption(
  ...
  await withTelemetry5("ai-setup", { cliOptions: mergedOptions }, async () => {
...
isAiCliFeatureEnabled() && registerAiMcpPassthrough(program, aiCommand, handleAiCommandFailure);
program.on("command:*", ([invalidCmd]) => {
 ...
 Did you mean ${import_picocolors.default.yellow(suggestion)}?`), logger4.error(errorMessage), process.exit(1);
});
