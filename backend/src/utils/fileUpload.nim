...
proc saveFile*(file: var UploadFile, uploadDirectory: string): string =
  if not dirExists(uploadDirectory):
    raise newException(
      FileNotFoundError, fmt "The directory '{uploadDirectory}' does not exist"
    )
  ...
  try:
    ...
    raise newException(
      IOError, fmt"Error saving file '{file.filename}' to '{uploadDirectory}':", e
    )
...
proc buildUniqueFilepath*(startFileName: string, targetDirectory: string): string =
  ...
  raise newException(
    ValueError,
    fmt"Was unable to build a unqiue filepath for '{startFileName}' in '{targetDirectory}' within {maxRenameAttempts} attempts!",
  )
...
proc deleteFile*(relativeFilePath: string, containingDirectory: string) =
  deleteFile(fmt"{containingDirectory}/{relativeFilePath}")

proc getRelativeFilepathTo*(
    absoluteFilepath: string, containingDirectory: string
): string =
