...
func jsonyResponse*(
    text: string, code = Http200, headers = initResponseHeaders(), version = HttpVer11
): Response {.inline.} =
...
proc outdatedUpdateResponse*[T: object | ref object](
    ctx: JWTContext, content: T
): Response {.inline.} =
  result = initResponse(
    HttpVer11, code = Http409, headers = initResponseHeaders(), body = content.toJson()
  )
  result.headers["Content-Type"] = "application/json"
