...
proc fromUnix*(input: int64): DjangoDateTime =
  times.fromUnix(input).local().DjangoDateTime

proc format*(
    x: DjangoDateTime,
    loc: DateTimeLocale = DefaultLocale,
    dateFormat = PRIMARY_DB_TIME_FORMAT,
): string =
  let trueDt = x.DateTime
  result = trueDt.format(dateFormat, loc)

proc now*(): DjangoDateTime =
  DjangoDateTime(times.now())
...
proc parseDefault*(input: string): DjangoDateTime =
  try:
    ...
    log(
      lvlDebug,
      fmt"Failed parsing '{input}' to format {PRIMARY_DB_TIME_FORMAT}. Attempting parsing with '{SECONDARY_DB_TIME_FORMAT}'",
    )
    try:
      ...
      log(
        lvlDebug,
        fmt"Failed parsing '{input}' to format {SECONDARY_DB_TIME_FORMAT}. Attempting parsing with '{OUTPUT_TIME_FORMAT}'",
      )
      ...
      try:
        ...
        log(
          lvlDebug,
          fmt"Failed parsing '{input}' to format {OUTPUT_TIME_FORMAT}. Attempting parsing with '{SESSION_DATE_FORMAT}'",
        )
        result = djangoDateTimeType.parse(input, SESSION_DATE_FORMAT, utc())

proc `>`*(a: DjangoDateTime, b: DjangoDateTime): bool =
