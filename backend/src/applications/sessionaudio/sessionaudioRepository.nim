...
proc getSessionAudioForCampaign*(
    connection: DbConn, campaignName: string
): seq[SessionAudioRead] =
  const condition =
    "session_id_campaign_id.name LIKE ? ORDER BY session_id.session_number DESC"
  result = connection.getList(SessionAudioRead, condition, campaignName.dbValue())

type SessionAudioNeighbours =
  tuple[
    priorNeighbour: Option[SessionAudioRead], nextNeighbour: Option[SessionAudioRead]
  ]

proc fetchSessionAudioNeighbourIds(
    connection: DbConn, campaignName: string, sessionNumber: int, isMainSession: bool
): seq[int64] =
  const sqlStatement =
    sql"""
...
proc isPriorEntry(
    startEntrySessionNumber: int,
    startEntryWasMainSession: bool,
    priorEntry: SessionAudioRead,
): bool =
  result =
    startEntrySessionNumber > priorEntry.session_id.session_number or
    startEntryWasMainSession > priorEntry.session_id.is_main_session

proc getSessionAudioNeighbours*(
    connection: DbConn, campaignName: string, sessionNumber: int, isMainSession: bool
): SessionAudioNeighbours =
  let neighbourIds =
    connection.fetchSessionAudioNeighbourIds(campaignName, sessionNumber, isMainSession)
  ...
  of 2:
    ...
    raise newException(
      DbError,
      fmt"Received {entries.len()} neighbours for a sessionaudio entry. There can at most only be 2 neighbours at once!",
    )
