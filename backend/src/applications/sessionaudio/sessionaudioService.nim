...
proc createSessionAudio*(
    connection: DbConn, requestParams: var SessionAudioDTO
): Option[SessionAudioRead] =
...
proc patchSessionAudio*(
    connection: DbConn, requestParams: var SessionAudioDTO, entry: var SessionAudio
): SessionAudio =
...
proc getSessionAudioByParams*(
    connection: DbConn, requestParams: ReadSessionAudioByParams
): SessionAudioRead =
  result = connection.getSessionAudio(
    requestParams.campaignName, requestParams.sessionNumber, requestParams.isMainSession
  )

proc getCampaignSessionAudio*(
...
proc moveAudioFile*(
    tmpFilePath: string, targetFileName: string, audioDirectory: string
): string =
  if not dirExists(audioDirectory):
    raise newException(
      FileNotFoundError, fmt"The media directory '{audioDirectory}' does not exist"
    )

  if not fileExists(tmpFilePath):
    raise newException(
      FileNotFoundError,
      fmt"The temporary audio file '{tmpFilePath}' could not be found! This is a technical error, it must be in the temporary file directory!",
    )
