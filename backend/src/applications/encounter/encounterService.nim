...
proc swapEncounterOrder*(encounter1Id: int64, encounter2Id: int64): JsonNode =
  ...
  if encounter1.diaryentry_id != encounter2.diaryentry_id:
    raise newException(
      DbError,
      fmt "The encouters with id {encounter1Id} and {encounter2Id} whose order is to be swapped are not from the same diaryentry!",
    )
...
proc createEncounter*(
    connection: DbConn, requestParams: CreateParams, entry: var Encounter
): Encounter =
  ...
  assert(
    entry.order_index.isSome() and entry.diaryentry_id != MODEL_INIT_ID and
      entry.title.isSome(),
  ...
  {.cast(gcsafe).}:
    connection.incrementOrderIndicesOfFollowingEncounters(
      entry.diaryentry_id, entry.order_index.get()
    )
    result = connection.createArticle(requestParams.body, Encounter)

proc cutInsertEncounter*(
    encounterId: int64, oldOrderIndex: int, newOrderIndex: int
): seq[EncounterRead] =
  withDbTransaction(connection):
    ...
    var affectedEncounters = connection.getEncountersBetweenOrderIndices(
      cutEncounter.diaryentry_id, oldOrderIndex, newOrderIndex
    )

    let isInsertAfterCut = oldOrderIndex < newOrderIndex
    let isForwardsInsert = isInsertAfterCut
    if isForwardsInsert:
      connection.updateEncounterOrderAfterForwardsInsert(
        affectedEncounters, cutEncounter
      )
    else:
      connection.updateEncounterOrderAfterBackwardsInsert(
        affectedEncounters, cutEncounter
      )
