...
proc cutInsertEncounter*(ctx: Context) {.async, gcsafe.} =
  ...
  respondOnError:
    let diaryentryEncounters: seq[EncounterRead] =
      cutInsertEncounter(cutEncounterId, oldOrderIndex, newOrderIndex)
...
proc createEncounterInDiaryentry*(ctx: Context) {.async, gcsafe.} =
  ...
  respondOnError:
    ...
    withDbTransaction(connection):
      ...
      let diaryentryEncounters: seq[EncounterRead] =
        connection.readDiaryEntryEncounters(createdEntry.diaryentry_id)
