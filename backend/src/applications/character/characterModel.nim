...
type Character* {.defaults, tableName: CHARACTER_TABLE.} = ref object of Model
  ...
  title*: Option[string] = some("")
    # A title the character might have, e.g. "Lord", "Duke", "Doctor"
  description*: Option[string] = some("") # A description of the character
  current_location_id* {.fk: Location.}: Option[int64] = some(MODEL_INIT_ID)
    # The id of the location at which the character is currently located
...
implDefaults(
  CharacterParentLocation, {DefaultFlag.defExported, DefaultFlag.defTypeConstr}
)
...
type CharacterLocation* {.defaults, readOnly, tableName: LOCATION_TABLE.} = ref object of Model
  ...
  parent_location_id*: Option[CharacterParentLocation] =
    some(new(CharacterParentLocation))
