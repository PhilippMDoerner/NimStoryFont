...
implDefaults(
  CharacterEncounterConnection, {DefaultFlag.defExported, DefaultFlag.defTypeConstr}
)

type CharacterEncounterRead* {.
  defaults, readOnly, tableName: ENCOUNTER_CHARACTER_TABLE
.} = ref object of Model
  encounter_id*: EncounterRead = new(EncounterRead)
  character_id*: Character = new(Character)

implDefaults(
  CharacterEncounterRead, {DefaultFlag.defExported, DefaultFlag.defTypeConstr}
)
