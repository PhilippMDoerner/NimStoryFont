...
proc findArticles*(
    ...
    searchText: string,
    searchLimit: int = 100,
    articleTypes: seq[ArticleType] = @[],
): seq[SearchSerializable] =
  let articleTables = articleTypes.mapIt(it.toTable())
  let searchHits: seq[SearchHit] =
    search(campaignName, searchText, searchLimit, articleTables)
