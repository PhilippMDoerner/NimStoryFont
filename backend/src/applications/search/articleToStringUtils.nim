...
proc getSearchBody*(connection: DbConn, modelInstance: DiaryEntry): string =
  result.add(
    fmt """{getSearchTitle(connection, modelInstance)} {modelInstance.title}"""
  )
  ...
  for encounter in encounters:
    result.add(
      fmt """{encounter.title.get(otherwise = "")} {encounter.description.get(otherwise = "")}"""
    )

proc getSearchBody*(connection: DbConn, modelInstance: Encounter): string =
  let diaryentry: EncounterDiaryentry =
    connection.getEntryById(modelInstance.diaryentry_id, EncounterDiaryentry)
  ...
  if modelInstance.location_id.isSome():
    let location: EncounterLocation =
      connection.getEntryById(modelInstance.location_id.get(), EncounterLocation)
...
proc getSearchBody*(connection: DbConn, modelInstance: Item): string =
  ...
  if modelInstance.owner_id.isSome():
    let owner: ItemOwner =
      connection.getEntryById(modelInstance.owner_id.get(), ItemOwner)
...
proc getSearchBody*(connection: DbConn, modelInstance: Map | MarkerMap): string =
  ...
  let mapLocations: seq[LocationRead] =
    connection.getManyToMany(modelInstance, MarkerRead, LocationRead)
...
proc getSearchBody*(
    connection: DbConn, modelInstance: Organization | OrganizationRead
): string =
  result.add(fmt """{modelInstance.name} """)
  result.add(fmt """{modelInstance.description.get(otherwise = "")} """)
  ...
  if modelInstance.headquarter_id.isSome():
    when modelInstance is Organization:
      let headquarterLocation = connection.getEntryById(
        modelInstance.headquarter_id.get(), OrganizationLocation
      )
  ...
  let members: seq[OrganizationCharacter] =
    connection.getManyFromOne(modelInstance, OrganizationCharacter)
...
proc getSearchBody*(connection: DbConn, modelInstance: Quest): string =
  ...
  if modelInstance.giver_id.isSome():
    let questGiver: QuestCharacter =
      connection.getEntryById(modelInstance.giver_id.get(), QuestCharacter)
    result.add(fmt """{questGiver.name} """)

  if modelInstance.taker_id.isSome():
    let questTaker: QuestCharacter =
      connection.getEntryById(modelInstance.taker_id.get(), QuestCharacter)
...
proc getSearchBody*(connection: DbConn, modelInstance: Rule): string =
  result.add(
    fmt """{modelInstance.name} {modelInstance.description.get(otherwise = "")}"""
  )
