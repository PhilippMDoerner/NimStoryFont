    ...
    try:
      receiver =
        if valueNode.str == "":
          ...
          some(valueNode.str.parseInt())
    except ValueError as e:
      raise newException(
        JsonParsingError,
        fmt"The field {typeName}.{fieldName} expected the json to have an int-type and received a string. That string was '{valueNode.str}' and could not be parsed into a number",
        e,
      )
  else:
    raise newException(
      JsonParsingError,
      fmt"The field {typeName}.{fieldName} expected the json to have an int-type, but the json was of kind '{valueNode.kind}'",
    )

template transferJsonIntRangeValue[T: range](
    receiver: var Option[T], valueNode: JsonNode
) =
  case valueNode.kind
  of JsonNodeKind.JInt:
...
template transferJsonInt64Value(receiver: var Option[int64], valueNode: JsonNode) =
  ...
  of JsonNodeKind.JString:
    try:
      receiver =
        if valueNode.str == "":
          ...
          some(valueNode.str.parseInt().int64)
    except ValueError as e:
      raise newException(
        JsonParsingError,
        fmt"The field {typeName}.{fieldName} expected the json to have an int-type and received a string. That string was '{valueNode.str}' and could not be parsed into a number",
    ...
    raise newException(
      JsonParsingError,
      fmt"The field {typeName}.{fieldName} expected the json to have an int64-type, but the json was of kind '{valueNode.kind}'",
    )
...
template transferJsonStringValue(receiver: var Option[string], valueNode: JsonNode) =
  ...
  of JsonNodeKind.JString:
    ...
    raise newException(
      JsonParsingError,
      fmt"The field {typeName}.{fieldName} expected the json to have a string-type, but the json was of kind '{valueNode.kind}'",
    )
...
template transferJsonBoolValue(receiver: var Option[bool], valueNode: JsonNode) =
  ...
  of JsonNodeKind.JInt:
    if valueNode.num in 0 .. 1:
      ...
      raise newException(
        JsonParsingError,
        fmt"The field {typeName}.{fieldName} expected the json to have a boolean-type or an int-value between 0 and 1. An int-value {valueNode.num} was received, but it was outside of that range!",
      )
  else:
    raise newException(
      JsonParsingError,
      fmt"The field {typeName}.{fieldName} expected the json to have a boolean-type, but the json was of kind '{valueNode.kind}'",
    )
...
template transferFloatValue(receiver: var Option[float], node: JsonNode) =
  ...
  of JsonNodeKind.JInt:
    ...
    raise newException(
      JsonParsingError,
      fmt"The field {typeName}.{fieldName} expected the json to have a float-type, but the json was of kind '{valueNode.kind}'",
    )
...
template transferDateTimeValue(receiver: var Option[DjangoDateTime], node: JsonNode) =
  ...
  of JsonNodeKind.JInt:
    ...
    raise newException(
      JsonParsingError,
      fmt"The field '{typeName}.{fieldName}' expects a DjangoDateTime value, which must be parsed from either a DateTime-string or a unix timestamp. The json was of kind '{valueNode.kind}'",
    )

template transferEnumValue[T: enum](receiver: var Option[T], valueNode: JsonNode) =
  case valueNode.kind
  of JsonNodeKind.JString:
    let enumVal: T = parseEnum[T](valueNode.str)
    receiver = some(enumVal)
  else:
    raise newException(
      JsonParsingError,
      fmt"The field '{typeName}.{fieldName}' expects an enum value, which must be parsed from a string. The json was of kind '{valueNode.kind}'",
    )

template moveValueIntoField[M: Model](
    entry: var M, fieldName: static string, value: Option[Parseable]
) =
  when entry.getField(fieldName).type() is Option:
    ...
    if value.isSome():
      ...
      raise newException(
        JsonParsingError,
        fmt"The field {typeName}.{fieldName} expected the json to have a value. But there was no value",
      )

proc transferJsonValue[M: Model](
    entry: var M,
    fieldName: static string,
    fieldType: typedesc[Parseable],
    valueNode: JsonNode,
) =
  ...
  if valueNode.kind == JsonNodeKind.JNull:
    when entry.getField(fieldName) is Option:
      ...
      raise newException(
        JsonParsingError,
        fmt"Attempted to set the field {typeName}.{fieldName} to null, but it is not an optional type!",
      )
  else:
...
proc mapJsonToModelFieldNames[T: Model](
    modelType: typedesc[T]
): Table[string, string] {.compileTime.} =
...
proc updateArticleWithJson*[T: Model](json: JsonNode, oldEntry: T): T =
  #TODO: Attach article type to this generic
  ...
  const jsonToModelFieldNameMap: Table[string, string] =
    mapJsonToModelFieldNames(T).invertTable()
  ...
  for modelFieldName, fieldValue in result[].fieldPairs:
    ...
    if json.hasKey(jsonFieldName):
      when fieldValue is Option:
        ...
        transferJsonValue(
          result, modelFieldName, fieldValue.type(), json[jsonFieldName]
        )
  ...
  if isBasedOnOutdatedUserData and hasChanges:
    raise newException(
      OutdatedDataError,
      "Tried updating a database entry that with data that has already been changed by another user!",
    )
