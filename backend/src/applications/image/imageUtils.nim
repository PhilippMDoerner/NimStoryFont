...
proc campaign_id*(entry: Image): int64 =
  ...
  elif (entry.item_article_id.isSome()):
    result = getEntryById(entry.item_article_id.get(), ImageItem).campaign_id
  elif (entry.location_article_id.isSome()):
    result = getEntryById(entry.location_article_id.get(), ImageLocation).campaign_id
  elif (entry.organization_article_id.isSome()):
    result =
      getEntryById(entry.organization_article_id.get(), ImageOrganization).campaign_id
  else:
    raise newException(
      InvalidImageError,
      fmt"An impossible image occurred that wasn't associated with any article type. Image Id was {entry.id}",
    )
