...
proc deleteImage*(imageId: int64) =
  deleteEntry(imageId, Image)
...
proc createImage*(connection: DbConn, imageDTO: var ImageDTO): Option[Image] =
  ...
  var relativeImagePath =
    absoluteImagePath.getRelativeFilepathTo(imageDTO.mediaDirectory)
...
proc updateImageFileOrName*(imageId: int64, imageDTO: var ImageDTO): Image =
  ...
  if isUpdateToNewImage:
    let newImageAbsoluteFilePath =
      saveFile(imageDTO.imageFile.get(), imageDTO.mediaDirectory)
    newImageRelativeFilePath =
      some(newImageAbsoluteFilePath.getRelativeFilepathTo(imageDTO.mediaDirectory))
  else:
  ...
  try:
    {.cast(gcsafe).}:
      withDbTransaction(connection):
        ...
        result = connection.updateImage(
          imageToUpdate, newImageRelativeFilePath, imageDTO.imageName
        )
  except DbError:
    if isUpdateToNewImage:
