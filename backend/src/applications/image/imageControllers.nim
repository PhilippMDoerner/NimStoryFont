  ...
  respondOnError:
    withDbConn(connection):
      ...
      if newImageEntry.isSome():
        let imageSerializable: ImageSerializable =
          connection.serializeImage(newImageEntry.get())
        resp jsonyResponse(ctx, imageSerializable)
      else:
        resp get400BadRequestResponse(
          "The sent image could not be saved, because there was no image file in the sent form under the 'image' key."
        )
