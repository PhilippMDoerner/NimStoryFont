...
proc addQuoteRoutes*(app: Prologue) =
  app.addRoute(
    re fmt"/quote/",
    handler =
      createCreateArticleHandler[CreateParams, Quote, QuoteSerializable](serializeQuote),
    httpMethod = HttpPost,
  ...
  app.addRoute(
    re fmt"/quote/pk/{ID_PATTERN}/",
    handler =
      createUpdateByIdHandler[UpdateParams, Quote, QuoteSerializable](serializeQuote),
    httpMethod = HttpPut,
    middlewares = @[loginMiddleware()],
  )
  ...
  app.addRoute(
    re fmt"/quote/pk/{ID_PATTERN}/",
    handler =
      createPatchByIdHandler[UpdateParams, Quote, QuoteSerializable](serializeQuote),
  ...
  app.addRoute(
    ...
    handler = createReadListHandler(
      getCharacterQuotes, checkQuoteListPermission, serializeQuoteReads
    ),
    httpMethod = HttpGet,
    middlewares = @[loginMiddleware()],
  )
