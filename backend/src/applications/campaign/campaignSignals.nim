...
import
  ../authentication/
    [authenticationService, authenticationConstants, authenticationModels]
import campaignUtils
import campaignModel
import ../core/signalSystem
import std/[options, sequtils, logging]

proc createCampaignPermission*(
    connection: DbConn, permissionName: string, codeName: string
): Permission =
  var campaignContentType = getEntryByField("model", "campaign", DjangoContentType)
  var permission = Permission(
    name: permissionName, codeName: codeName, content_type_id: campaignContentType.id
  )
...
proc createGuestPermission*(connection: DbConn, instance: var Campaign) =
  ...
  let permission = connection.createCampaignPermission(
    instance.guestPermissionName(), instance.guestPermissionCodename()
  )

  instance.guest_permission_id = some(permission.id)

proc createMemberPermission*(connection: DbConn, instance: var Campaign) =
  let permission = createCampaignPermission(
    connection, instance.memberPermissionName(), instance.memberPermissionCodename()
  )
...
proc createAdminPermission*(connection: DbConn, instance: var Campaign) =
  let permission = createCampaignPermission(
    connection, instance.adminPermissionName(), instance.adminPermissionCodename()
  )
...
proc addCampaignPermissions(
    connection: DbConn, group: Group, permissionCodeNames: seq[string]
) =
...
proc createGuestGroup(connection: DbConn, instance: var Campaign) =
  ...
  let guestPermissionCodenames: seq[string] =
    concat(CAMPAIGN_GUEST_PERMISSIONS, @[instance.guestPermissionCodename()])
...
proc createMemberGroup(connection: DbConn, instance: var Campaign) =
  ...
  let memberPermissionCodenames: seq[string] =
    concat(CAMPAIGN_MEMBER_PERMISSIONS, @[instance.memberPermissionCodename])
...
proc createAdminGroup(connection: DbConn, instance: var Campaign) =
  ...
  let adminPermissionCodenames: seq[string] =
    concat(CAMPAIGN_ADMIN_PERMISSIONS, @[instance.adminPermissionCodename])
  addCampaignPermissions(connection, adminGroup, adminPermissionCodenames)

  instance.admin_group_id = some(adminGroup.id)

proc createCampaignGroupsAndPermissions(
    connection: DbConn, modelInstance: var Campaign
) =
