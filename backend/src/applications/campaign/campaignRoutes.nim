...
proc addCampaignRoutes*(app: Prologue) =
  ...
  app.addRoute(
    re fmt"/campaign/pk/{ID_PATTERN}/",
    handler = createReadByIdHandler[ReadByIdParams, Campaign, CampaignSerializable](
      serializeCampaign
    ),
    httpMethod = HttpGet,
    middlewares = @[loginMiddleware()],
  ...
  app.addRoute(
    ...
    handler = createReadListHandler(
      getAllCampaignOverviews, noCampaignListPermissionCheck, overviewSerialize
    ),
    httpMethod = HttpGet,
    middlewares = @[loginMiddleware()],
  )
  ...
  app.addRoute(
    re fmt"/campaign/",
    handler = createReadListHandler(
      getAllCampaignReads, noCampaignListPermissionCheck, serializeCampaignReads
    ),
    httpMethod = HttpGet,
    middlewares = @[loginMiddleware()],
  )
  ...
  app.addRoute(
    ...
    handler = createReadHandler[CampaignNameParams, CampaignRead, CampaignSerializable](
      readCampaignByName, checkAdminPermission, serializeCampaignRead
    ),
    httpMethod = HttpGet,
    middlewares = @[loginMiddleware()],
  )
  ...
  app.addRoute(
    ...
    handler = createCreateHandler[
      CreateParams, EmptySearchResponse, CampaignSerializable
    ](checkCampaignAdminPermission, createEntry, serializeEmptySearchResponse),
    httpMethod = HttpPost,
    middlewares = @[loginMiddleware()],
  )
  ...
  app.addRoute(
    ...
    handler = createDeleteHandler[DeleteParams, EmptySearchResponse](
      readArticleById, checkCampaignAdminPermission, deleteArticle
    ),
    httpMethod = HttpDelete,
    middlewares = @[loginMiddleware()],
  )
