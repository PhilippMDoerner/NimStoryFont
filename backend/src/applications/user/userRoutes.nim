...
proc addUserRoutes*(app: Prologue) =
  app.addRoute(
    re fmt"/user/",
    handler = createCreateHandler[CreateParams, User, UserSerializable](
      checkAdminPermission, createUser, serializeUser
    ),
    httpMethod = HttpPost,
  ...
  app.addRoute(
    ...
    handler = createDeleteHandler[DeleteParams, User](
      readUserById, checkUserDeletePermission, deleteUser
    ),
  ...
  app.addRoute(
    ...
    handler = createReadHandler[ReadByIdParams, User, UserSerializable](
      readUserById, checkUserDeletePermission, serializeUser
    ),
    httpMethod = HttpGet,
    middlewares = @[loginMiddleware()],
  )
  ...
  app.addRoute(
    ...
    handler = createReadListHandler[ReadListParams, User, UserSerializable](
      getCampaignUserListOverview, checkCampaignReadListPermission, serializeUsers
    ),
    httpMethod = HttpGet,
    middlewares = @[loginMiddleware()],
  )
