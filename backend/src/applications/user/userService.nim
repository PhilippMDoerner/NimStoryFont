...
proc readUserById*(
    connection: DbConn, requestParams: ReadByIdParams | UpdateParams
): User =
...
proc getCampaignUserListOverview*(
...
proc createUser*(
    connection: DbConn, requestParams: CreateParams, newEntry: var User
): User =
...
proc updateUser*(connection: DbConn, requestData: UpdateParams): User =
  ...
  assert(
    entry.id == requestData.id,
    "Tried updating {modelType.name()} and change id from {entryId} to {entry.id}!",
  )

  result = connection.updateEntryInTransaction(entry)

proc updateUserGroups*(
    connection: DbConn, requestData: UpdateParams, entry: User
): User =
  let parsedBody = requestData.body.parseJson()

  let newGroupIds: seq[int64] =
    parsedBody["groups"].elems.map(idNode => idNode.getInt().int64)
  let oldUserGroups: seq[UserGroup] = connection.getManyFromOne(entry, UserGroup)
  let oldGroupIds: HashSet[int64] =
    oldUserGroups.map(userGroup => userGroup.group_id.id).toHashSet()
...
proc updateUserPassword*(
    connection: DbConn, user: var User, newPassword: string
): User =
...
proc updateUserPassword*(
    connection: DbConn, username: string, newPassword: string
): User =
...
proc patchUser*(connection: DbConn, requestData: UpdateParams, entry: User): User =
  ...
  if isGroupUpdatePatch:
    ...
    raise newException(
      ValueError,
      "Password updates are not allowed via patch on user. Use /authdata/patch-password",
    )
  else:
...
proc getUserMetadataByCategory*(
    connection: DbConn, userId: int64, category: string
): seq[UserMetadata] =
  return connection.fetchUserMetadataByCategory(userId, category)

proc createUserMetadata*(
    connection: DbConn, requestParams: CreateParams, newEntry: var UserMetadata
): UserMetadata =
  ...
  if hasEntryAlready:
    raise newException(
      DuplicateEntryError,
      fmt"'{newEntry.name}' userMetadata entry in category '{newEntry.category}' already exists",
    )
