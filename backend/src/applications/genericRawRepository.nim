...
proc rawSelectRows*[T: RawType](
    connection: DbConn,
    sqlQuery: string,
    outputType: typedesc[T],
  ...
  when T is ref object:
    ...
    if hasRows:
      ...
      if not hasSingleColumn:
        raise newException(
          ValueError,
          fmt"Tried parsing sqlQuery result into type '{name(T)}'. T expected to be a primitive datatype and thus require only 1 column of data. However, the query returned '{row[0].len()}' columns of data! Query: {sqlQuery}",
        )
      else:
        for row in rows:
          let columnVal = row[0]
...
proc rawSelectRow*[T: RawType](
  ...
  when T is ref object:
    ...
    if row.isNone():
      raise newException(
        NotFoundError,
        fmt"Expected to find row for SQL query '{sqlQuery}' <- '{queryParams}', but found nothing!",
      )
    else:
      let rawRow = row.get()
      ...
      if not hasSingleColumn:
        ...
        raise newException(
          ValueError,
          fmt"Tried parsing sqlQuery into type '{typeName}'. T expected to be a primitive datatype and thus require only 1 column of data. However, the query returned '{rawRow.len()}' columns of data! Query: {sqlQuery}",
        )
      else:
...
proc rawExec*(
    connection: DbConn, sqlQuery: string, queryParams: varargs[DbValue]
) {.gcsafe.} =
  when defined(normDebug):
...
proc assertNoNestedObjects[T: ref object](t: typedesc[T]): bool {.compileTime.} =
  for fieldName, value in T()[].fieldPairs:
    ...
    assert(
      not isObject,
      fmt"You're trying to insert with a non-Model object with nested fields with Model '{name}'. This is not allowed. Please ensure all fields are in there directly",
    )

proc rawInsert*[T: ref object](connection: DbConn, obj: T, tableName: string) =
  static:
    discard assertNoNestedObjects(T)
