...
proc getRecentlyUpdatedArticles*(
    campaignName: string, pageNumber: int, pageSize: int
): seq[JsonNode] =
  let recentlyUpdatedArticlesHits: seq[UpdatedArticle] =
    getRecentlyUpdatedArticleViewEntries(campaignName, pageNumber, pageSize)
...
proc getAllRecentlyUpdatedArticles*(con: DbConn, campaignName: string): seq[JsonNode] =
  let recentlyUpdatedArticlesHits: seq[UpdatedArticle] =
    con.getAllRecentlyUpdatedArticleViewEntries(campaignName)
