...
func dbType*(T: typedesc[FontAwesomeType]): string =
  "TEXT"
func dbValue*(val: FontAwesomeType): DbValue =
  dbValue($val)
func to*(dbVal: DbValue, T: typedesc[FontAwesomeType]): FontAwesomeType =
  ...
  if not isStringValue:
    raise newException(
      ValueError,
      "Expected string value for FontAwesomeType column from database, but received {$dbVal.kind}",
    )
