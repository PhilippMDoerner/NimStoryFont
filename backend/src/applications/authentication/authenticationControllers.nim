...
proc setAuthCookies(
    response: var Response, accessToken, refreshToken: TokenSerializable
) =
  response.setCookie(
    "refreshToken",
    refreshToken.token,
...
proc createAndSerializeAuthData(
    connection: DbConn, ctx: Context, user: User
): AuthDataSerializable =
  let accessTokenLifetimeInDays: int =
    ctx.getSetting(SettingName.snAccesTokenLifetime).getInt()
  let refreshTokenLifetimeInDays: int =
    ctx.getSetting(SettingName.snRefreshTokenLifetime).getInt()
  ...
  result = serializeLoginData(
    userContainer, accessToken, accessTokenLifetimeInDays, refreshToken,
...
proc refreshTokens*(ctx: Context) {.async.} =
  ...
  respondOnError:
    withDbConn(connection):
      let oldAuthenticationData: TokenData =
        connection.getRefreshTokenData(tokenLifetime, refreshToken)
      ...
      let newAuthData: AuthDataSerializable =
        connection.createAndSerializeAuthData(ctx, user)
  ...
  respondOnError:
    withDbConn(connection):
      let hasActiveWorkflow = connection.hasActiveWorkflowConfirmation(
        user_id, WorkflowType.wtPASSWORD_RESET, workflowLifetimeInSeconds
      )
      ...
      let authData: AuthDataSerializable =
        connection.createAndSerializeAuthData(ctx, user)
...
proc login*(ctx: Context) {.async.} =
  ...
  respondOnError:
    withDbConn(connection):
      let loginData: AuthDataSerializable =
        connection.createAndSerializeAuthData(ctx, user)
...
proc startPasswordResetWorkflow*(ctx: Context) {.async, gcsafe.} =
  ...
  withDbConn(connection):
    ...
    let dto = WorfklowStartResetDTO(user: user, workflow: WorkflowType.wtPASSWORD_RESET)
    try:
...
proc confirmPasswordReset*(ctx: Context) {.async.} =
  ...
  if canConfirm:
    ...
    withDbConn(connection):
      try:
        ...
        ctx.response.setAuthCookies(authData.accessToken, authData.refreshToken)
  ...
  var redirectQueryParams =
    @[("source", $WorkflowType.wtPASSWORD_RESET), ("state", $workflowState)]
  if workflowState == WorkflowState.wsSuccess and not confirmation.isNil():
    let creationUnixTimestamp =
      DateTime(confirmation.creation_datetime).toTime().toUnix().int64
