...
proc getCampaignIdMembershipTable(
    connection: DbConn
): Table[string, CampaignMemberships] =
  ...
  for campaign in campaigns:
    if campaign.guest_group_id.isSome():
      ...
      membershipTable.addCampaignGroup(
        guestGroupName, campaign.id, CampaignAccessLevel.GUEST
      )
      membershipTable.addCampaignGroup(
        guestGroupName, campaign.name, CampaignAccessLevel.GUEST
      )

    if campaign.member_group_id.isSome():
      let memberGroupName: string = campaign.member_group_id.get().name
      membershipTable.addCampaignGroup(
        memberGroupName, campaign.id, CampaignAccessLevel.MEMBER
      )
      membershipTable.addCampaignGroup(
        memberGroupName, campaign.name, CampaignAccessLevel.MEMBER
      )

    if campaign.admin_group_id.isSome():
      let adminGroupName: string = campaign.admin_group_id.get().name
      membershipTable.addCampaignGroup(
        adminGroupName, campaign.id, CampaignAccessLevel.ADMIN
      )
      membershipTable.addCampaignGroup(
        adminGroupName, campaign.name, CampaignAccessLevel.ADMIN
      )
...
proc getUserCampaignMemberships(connection: DbConn, user: User): CampaignMemberships =
  ...
  let campaignIdMembershipTable: Table[string, CampaignMemberships] =
    connection.getCampaignIdMembershipTable()
...
proc getPermissions*(connection: DbConn, codeNames: seq[string]): seq[Permission] =
  ...
  let partialConditions: seq[string] = codeNames.map(
    proc(s: string): string =
      fmt """{permissionTable}.codename = "{s}" """
  )
...
proc addCampaignMember*(
    connection: DbConn, campaign: CampaignRead, role: CampaignRole, newMember: var User
) =
...
proc removeCampaignMember*(
    connection: DbConn, campaign: CampaignRead, role: CampaignRole, member: var User
) =
  let campaignGroup: Group = getCampaignGroupForRole(campaign, role)
  connection.deleteGroupMembership(member.id, campaignGroup.id)

proc sendPasswordResetConfirmationRequestEmail*(
    workflowDto: WorfklowStartResetDTO, confirmationRequestEntry: Confirmation
) {.async.} =
  if workflowDto.user.email == "":
    raise newException(
      MissingEmailError,
      fmt"User '{workflowDto.user.username}' does not have an email address",
    )
  ...
  let (subject, body) = getPasswordResetConfirmationRequestEmail(
    domain, workflowDto.user.username, workflowDto.user.id,
...
proc sendPasswordResetEmail*(
    user: User, newPassword: string, settings: Settings
) {.async.} =
  if user.email == "":
    raise newException(
      MissingEmailError, fmt"User '{user.username}' does not have an email address"
    )
...
proc getAccessTokenData*(
    connection: DbConn, tokenLifetimeInDays: int64, token: string
): TokenData =
  result = connection.getTokenData(tokenLifetimeInDays, token, access)

proc getRefreshTokenData*(
    connection: DbConn, tokenLifetimeInDays: int64, token: string
): TokenData =
...
proc createAuthToken*(
    connection: DbConn, userId: int64, tokenType: TokenType
): TokenContainer =
  ...
  for i in 0 .. 1000:
    ...
    try:
      ...
      let isDuplicateToken: bool =
        getCurrentExceptionMsg() == "UNIQUE constraint failed: authtoken_token.key"
      if isDuplicateToken:
        continue
      else:
        raise
...
proc createAuthToken*(
    connection: DbConn, user: User, tokenType: TokenType
): TokenContainer =
  result = connection.createAuthToken(user.id, tokenType)

proc createConfirmationRequest*(
    connection: DbConn, workflowDto: WorfklowStartResetDTO
): Confirmation =
  let token = generateToken()
...
proc getCurrentConfirmationState*(
    connection: DbConn, workflowDto: WorkflowConfirmDTO
): Confirmation =
  result = connection.getWorkflowConfirmation(
    workflowDto.user_id, workflowDto.token, workflowDto.workflow,
