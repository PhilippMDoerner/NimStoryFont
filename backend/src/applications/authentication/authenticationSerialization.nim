...
proc serializeGroup*(connection: DbConn, entry: Group): GroupSerializable =
  let groupPermissionIds: seq[int64] = connection
    .getManyFromOne(entry, GroupPermission)
    .map(joinEntry => joinEntry.permission_id)
  result =
    GroupSerializable(id: entry.id, name: entry.name, permissions: groupPermissionIds)
...
proc serializeToken(
    tokenData: TokenContainer, tokenLifetimeInDays: int
): TokenSerializable =
