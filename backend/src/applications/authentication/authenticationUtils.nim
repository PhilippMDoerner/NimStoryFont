...
proc checkCampaignWritePermission(ctx: JWTContext, entryCampaignId: int64) =
  ...
  if not ctx.hasCampaignMembership(entryCampaignId):
    raise newException(
      ...
      fmt"You are not a member of the campaign '{entryCampaignId}' which you're creating an entry for",
    )

  let accessLevel: CampaignAccessLevel = ctx.getCampaignAccessLevel(entryCampaignId)
  let hasWritePermission =
    accessLevel in [CampaignAccessLevel.MEMBER, CampaignAccessLevel.ADMIN]
  if not hasWritePermission:
    raise newException(
      CampaignPermissionError,
      "Only members and admins of this campaign can create/update/delete entries",
    )
...
proc checkAdminPermission*[T: Model](ctx: JWTContext, entry: T) =
  ...
  if not hasAdminPermission:
    raise newException(
      AdminPermissionError, "Only admins of the webpage can perform this action"
    )
...
proc checkSuperUserPermission*[T: Model](ctx: JWTContext, entry: T) =
  ...
  if not hasSuperUserPermission:
    raise newException(
      AdminPermissionError, "Only superusers of the webpage can perform this action"
    )
...
proc checkReadListPermission*(ctx: JWTContext, campaignName: string) =
  ...
  if not ctx.hasCampaignMembership(campaign.id):
    log(
      lvlDebug,
      fmt"'{ctx.tokenData.username}' does not have access to campaign '{campaignName}'",
    )
    raise newException(
...
proc checkCampaignReadListPermission*[T: Model](ctx: JWTContext, entries: seq[T]) =
  ...
  if campaignName.isNone():
    ...
    const errorMsg =
      fmt"Tried to check read list permission on route for a campaign, but the path contained no parameter '{campaignNameParam}'!"
