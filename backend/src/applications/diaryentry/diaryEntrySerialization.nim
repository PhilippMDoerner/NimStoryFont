...
proc serializeToDiaryEntryStub(
    connection: DbConn, entry: DiaryEntryRead
): DiaryEntryStub =
  ...
  result = DiaryEntryStub(
    ...
    author_details: DiaryEntryAuthorSerializable(
      pk: entry.author_id.id, name: entry.author_id.username
    ),
  )

proc getAdjacentDiaryEntries(
    connection: DbConn, entry: DiaryEntryRead
): AdjacentDiaryEntries =
  ...
  var diaryentries: seq[DiaryEntryRead] =
    connection.getDiaryEntriesForCampaign(entry.session_id.campaign_id.name)
  if diaryentries.len() == 0:
    return AdjacentDiaryEntries(
      prior_diaryentry: none(DiaryEntryStub), next_diaryentry: none(DiaryEntryStub)
    )
  ...
  result = AdjacentDiaryEntries(
    prior_diaryentry:
      priorDiaryEntry.map(entry => connection.serializeToDiaryEntryStub(entry)),
    next_diaryentry:
      nextDiaryEntry.map(entry => connection.serializeToDiaryEntryStub(entry)),
  )

proc serializeDiaryEntryRead*(
    connection: DbConn, entry: DiaryEntryRead
): DiaryEntrySerializable =
  let session = connection.getEntryById(entry.session_id.id, SessionRead)
  let serializedSession = connection.serializeSessionRead(session)

  let encounters = connection.getManyFromOne(entry, EncounterRead)
  let encounterParentLocations: Table[int64, seq[Location]] =
    connection.getDiaryEntryEncounterLocations(encounters)
  let encounterCharacterConnections: Table[int64, seq[CharacterEncounterRead]] =
    connection.getManyFromOne(encounters, CharacterEncounterRead, "encounter_id")
  var serializedEncounters: seq[EncounterSerializable] = @[]
  for encounter in encounters:
    let parentLocations: seq[Location] =
      if encounterParentLocations.hasKey(encounter.id):
        encounterParentLocations[encounter.id]
      else:
        @[]
    let connections: seq[CharacterEncounterRead] =
      if encounterCharacterConnections.hasKey(encounter.id):
        encounterCharacterConnections[encounter.id]
      else:
        @[]

    serializedEncounters.add(
      serializeEncounterRead(encounter, connections, parentLocations)
    )
...
proc serializeDiaryEntry*(
    connection: DbConn, entry: DiaryEntry
): DiaryEntrySerializable =
...
proc serializeDiaryEntrySession*(
    entry: DiaryEntrySession
): DiaryEntrySessionSerializable =
...
proc overviewSerialize*(
    entry: DiaryEntryRead, encounters: seq[EncounterRead]
): DiaryEntryOverviewSerializable =
  let firstEncounter: Option[EncounterRead] =
    if encounters.len() > 0:
      some(encounters[0])
    else:
      none(EncounterRead)
  let firstEncounterText: Option[string] =
    firstEncounter.map(enc => enc.description).flatten()
...
proc overviewSerialize*(
    connection: DbConn, entries: seq[DiaryEntryRead]
): seq[DiaryEntryOverviewSerializable] =
  let allEncounters: Table[int64, seq[EncounterRead]] =
    connection.getManyFromOne(entries, EncounterRead, "diaryentry_id")
