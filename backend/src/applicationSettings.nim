...
type SettingName* = enum
  ...
  snConnectionLimit = "databaseConnectionLimit"
    # Number of connections that shall be available in the database connection pool 
  ...
  snAudioUploadDir = "audioUploadDir"
    # Sub-Folder of snAudioDir. Directory that newly uploaded audio files shall be moved to
  snPageSize = "pageSize"
    # The amount of entries to send to the frontend as one "page" if number of entries sent must be limited
  snSmtpName = "smtp"
  snSmtpPort = "smtpPort"
...
const INT_SETTINGS = [
  snAccesTokenLifetime, snRefreshTokenLifetime, snConnectionLimit, snPageSize,
  snSmtpPort,
].toHashSet
const STRING_SETTINGS = [
  snSettingSetName, snSecretKey, snDatabasePath, snBaseDir, snImageDir, snAudioDir,
  snAudioUploadDir, snSmtpName, snEmailName, snEmailPassword, snEmailDomain,
  snServerDomain,
].toHashSet()

proc getSetting*(ctx: Context, setting: SettingName): JsonNode =
  ctx.getSettings($setting)

proc getCoreSetting(settings: Settings, setting: CoreSettingName): JsonNode =
  settings.getOrDefault("prologue")[$setting]
...
proc validateSettingValue*(settingName: SettingName, settingValue: JsonNode) =
  if INT_SETTINGS.contains(settingName):
    if settingValue.kind != JInt:
      raise newException(
        ValueError,
        fmt"Setting '{settingName}' with value '{settingValue}' was expected to be an integer, but was of type '{settingValue.kind}'!",
      )
  elif STRING_SETTINGS.contains(settingName):
    if settingValue.kind != JString:
      raise newException(
        ValueError,
        fmt"Setting '{settingName}' with value '{settingValue}' was expected to be a string, but was of type '{settingValue.kind}'!",
      )
  else:
    raise newException(
      ValueError,
      fmt"Type of setting '{settingName}' has not been defined! A setting with this name might not exist.",
    )
...
proc validateSettings*(settings: Settings) =
  for setting in SettingName:
    ...
    if settingValue == nil:
      raise newException(
        SettingsFileError,
        fmt"The loaded settings file at '{DEFAULT_SETTINGS_FILE_PATH}' is missing a value for the setting '{setting}'!",
      )
...
proc getSettingsFilepath(): string =
  if existsEnv(SETTINGS_FILE_PATH_ENVIRONMENT_VARIABLE):
    ...
    if filepath == "":
      raise newException(
        ValueError,
        fmt"The environment variable '{SETTINGS_FILE_PATH_ENVIRONMENT_VARIABLE}' denotes the location of the settings file for nimstoryfont. It can not be an empty string. Either delete it to use the default location '{DEFAULT_SETTINGS_FILE_PATH}' or provide a valid path to a nimstoryfont settings file.",
      )

    if not fileExists(filepath):
      raise newException(
        ValueError,
        fmt"The settings file '{filepath}' does not exist. It was provided by the environment variable '{SETTINGS_FILE_PATH_ENVIRONMENT_VARIABLE}'.",
      )
...
#Constants
const PRIMARY_DB_TIME_FORMAT* = "yyyy-MM-dd HH:mm:ss'.'ffffff"
  #Datetime format as Django stores it in DB
const SECONDARY_DB_TIME_FORMAT* = "yyyy-MM-dd HH:mm:ss"
  # Datetime format as Django sometimes stores it in DB
...
const USER_CONFIRMATION_TABLE* = "user_confirmation_pending"
