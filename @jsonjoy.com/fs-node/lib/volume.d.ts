...
export declare class Volume implements FsCallbackApi, FsSynchronousApi {
    ...
    watch(path: PathLike, options: 'buffer' | (IWatchOptions & {
        encoding: 'buffer';
    }), listener?: (eventType: string, filename: Buffer) => void): FSWatcher;
    watch(path: PathLike, options: IWatchOptions | string | undefined, listener?: (eventType: string, filename: string) => void): FSWatcher;
...
export declare class FSWatcher extends EventEmitter {
     ...
     */
    unref(): this;
    start(path: PathLike, persistent?: boolean, recursive?: boolean, encoding?: BufferEncoding, ignore?: opts.TWatchIgnorePattern | opts.TWatchIgnorePattern[]): void;
    close(): void;
}
