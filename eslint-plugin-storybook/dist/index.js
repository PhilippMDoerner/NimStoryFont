import CJS_COMPAT_NODE_URL_s8nn1pfubvq from 'node:url';
import CJS_COMPAT_NODE_PATH_s8nn1pfubvq from 'node:path';
import CJS_COMPAT_NODE_MODULE_s8nn1pfubvq from "node:module";

var __filename = CJS_COMPAT_NODE_URL_s8nn1pfubvq.fileURLToPath(import.meta.url);
var __dirname = CJS_COMPAT_NODE_PATH_s8nn1pfubvq.dirname(__filename);
var require = CJS_COMPAT_NODE_MODULE_s8nn1pfubvq.createRequire(import.meta.url);
...
// package.json
var name = "eslint-plugin-storybook", version = "10.5.7";
...
var docsUrl = (ruleName) => `https://github.com/storybookjs/storybook/blob/next/code/lib/eslint-plugin/docs/rules/${ruleName}.md`, getMetaObjectExpression = (node, context) => {
  let meta2 = node.declaration, { sourceCode } = context;
  if (isIdentifier(meta2)) {
    let scope = sourceCode.getScope ? sourceCode.getScope(node) : context.getScope(), variable = ASTUtils.findVariable(scope, meta2.name), decl = variable && variable.defs.find((def) => isVariableDeclarator(def.node));
    decl && isVariableDeclarator(decl.node) && (meta2 = decl.node.init);
  }
  return (isTSAsExpression(meta2) || isTSSatisfiesExpression(meta2)) && (meta2 = meta2.expression), isObjectExpression(meta2) ? meta2 : null;
...
var await_interactions_default = createStorybookRule({
  ...
  create(context) {
    let FUNCTIONS_TO_BE_AWAITED = [
      ...
      let shouldAwait = (name2) => FUNCTIONS_TO_BE_AWAITED.includes(name2) || name2.startsWith("findBy");
  ...
  create(context) {
    return {
      ExportDefaultDeclaration(node) {
        let meta2 = getMetaObjectExpression(node, context);
        if (!meta2)
          return null;
        meta2.properties.find(
...
var default_exports_default = createStorybookRule({
  ...
  create(context) {
    let getComponentName = (node, filePath) => {
      let name2 = path.basename(filePath).split(".")[0];
      return node.body.find((stmt) => {
        if (isImportDeclaration(stmt) && isLiteral(stmt.source) && stmt.source.value.startsWith(`./${name2}`))
          return !!stmt.specifiers.find(
            (spec) => isIdentifier(spec.local) && spec.local.name === name2
          );
      }) ? name2 : null;
    ...
    return {
      ...
      "Program:exit": function(program) {
        if (!isCsf4Style && !hasDefaultExport && !hasStoriesOfImport) {
          let componentName = getComponentName(program, context.filename), node = program.body.find((n) => !isImportDeclaration(n)) || program.body[0] || program, report = {
            node,
  ...
  create: function(context) {
    return {
      ExportDefaultDeclaration: function(node) {
        ...
        if (!meta2)
          return null;
        let titleNode = meta2.properties.find(
...
var meta_satisfies_type_default = createStorybookRule({
  ...
  create(context) {
    let sourceCode = context.sourceCode, getTextWithParentheses = (node) => {
      let beforeCount = 0, afterCount = 0;
      if (ASTUtils.isParenthesized(node, sourceCode)) {
        let bodyOpeningParen = sourceCode.getTokenBefore(node, ASTUtils.isOpeningParenToken), bodyClosingParen = sourceCode.getTokenAfter(node, ASTUtils.isClosingParenToken);
      ...
      if (parent)
        switch (parent.type) {
          ...
          case AST_NODE_TYPES2.TSAsExpression:
            return (fixer) => [
              fixer.replaceText(parent, getTextWithParentheses(meta2)),
    ...
    return {
      ExportDefaultDeclaration(node) {
        ...
        if (!meta2)
          return null;
        (!meta2.parent || !isTSSatisfiesExpression(meta2.parent)) && context.report({
          node: meta2,
      ...
      ExportNamedDeclaration: function(node) {
        ...
        if (isVariableDeclaration(decl)) {
          ...
          if (isIdentifier(id) && isObjectExpression(init)) {
            ...
            if (!storyNameNode)
              return;
            let { name: name2 } = id, resolvedStoryName = storyNameFromExport(name2);
          ...
          suggest: [
            {
              ...
              fix(fixer) {
                let hasComma = context.sourceCode.text.slice(
                  titleNode.range[0],
                  titleNode.range[1] + 1
...
var prefer_pascal_case_default = createStorybookRule({
  ...
  create(context) {
    let isPascalCase = (str) => /^[A-Z]+([a-z0-9]?)+/.test(str), toPascalCase = (str) => str.replace(new RegExp(/[-_]+/, "g"), " ").replace(new RegExp(/[^\w\s]/, "g"), "").replace(
      ...
      let { name: name2 } = id;
      if (!isExportStory2(name2, nonStoryExportsConfig2) || name2 === "__namedExportsOrder")
        return null;
      !name2.startsWith("_") && !isPascalCase(name2) && context.report({
        node: id,
        ...
        suggest: [
          {
            ...
            *fix(fixer) {
              let suffix = context.sourceCode.text.slice(id.range[0], id.range[1]).substring(name2.length), pascal = toPascalCase(name2);
              yield fixer.replaceTextRange(id.range, pascal + suffix);
              let scope = getModuleScope();
              if (scope) {
                let variable = ASTUtils.findVariable(scope, name2), referenceCount = variable?.references?.length || 0;
    ...
    }, meta2, nonStoryExportsConfig, namedExports = [], hasStoriesOfImport = !1;
    return {
      ImportSpecifier(node) {
        "name" in node.imported && node.imported.name === "storiesOf" && (hasStoriesOfImport = !0);
      },
      ExportDefaultDeclaration: function(node) {
        if (meta2 = getMetaObjectExpression(node, context), meta2)
          try {
            nonStoryExportsConfig = {
              excludeStories: getDescriptor(meta2, "excludeStories"),
              includeStories: getDescriptor(meta2, "includeStories")
            };
          } catch {
          }
      },
...
var story_exports_default = createStorybookRule({
  ...
  create(context) {
    let hasStoriesOfImport = !1, nonStoryExportsConfig = {}, meta2, namedExports = [];
    return {
      ImportSpecifier(node) {
        "name" in node.imported && node.imported.name === "storiesOf" && (hasStoriesOfImport = !0);
      },
      ...
      "Program:exit": function(program) {
        if (hasStoriesOfImport || !meta2 || namedExports.filter(
...
var use_storybook_testing_library_default = createStorybookRule({
  ...
  meta: {
    ...
    messages: {
      ...
      dontUseTestingLibraryDirectly: "Do not use `{{library}}` directly in the story. You should import the functions from `storybook/test` instead."
    }
  },
  create(context) {
    let getRangeWithoutQuotes = (source) => [
      ...
      if (!previousSpecifier)
        return null;
      let end = previousSpecifier.range[1], fullText = context.sourceCode.text, importEnd = node.range[1], closingBrace = fullText.indexOf("}", end - 1);
    ...
    return {
      ImportDeclaration(node) {
        node.source.value.includes("@testing-library") && context.report({
          ...
          *fix(fixer) {
            if (yield fixer.replaceTextRange(getRangeWithoutQuotes(node.source), "storybook/test"), hasDefaultImport(node.specifiers)) {
