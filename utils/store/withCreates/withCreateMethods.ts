...
type NewMethods<Name extends string, Q> =
  Q extends Request<void, infer Resp extends object>
    ? Record<`create${Capitalize<Name>}`, () => Observable<Resp>>
...
export function withCreateMethods<Requests extends RequestMap>(
  requests: Requests,
) {
  return signalStoreFeature(
    withMethods((store) => {
      ...
      const createFunctions = Object.keys(requests)
                ...
                tapResponse({
                  next: (val) => {
                    toastService.addToast(
                      successToast('Created Article successfully!'),
                    );
                    patchState(store, {
                      [keys.dataField]: val,
