 ...
 */
function generateDebugId(name) {
    const sha = (0, node_crypto_1.createHash)('sha1').update(ANGULAR_BUILD_NAMESPACE).update(name).digest();
 ...
 */
function injectDebugIdIntoJs(text, id) {
 ...
 */
function injectDebugIdIntoSourceMap(json, id) {
    let parsed;
    try {
