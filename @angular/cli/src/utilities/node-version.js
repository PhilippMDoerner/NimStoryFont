...
exports.supportedNodeVersions = void 0;
exports.isNodeVersionSupported = isNodeVersionSupported;
exports.isNodeVersionMinSupported = isNodeVersionMinSupported;
/**
 ...
 */
exports.supportedNodeVersions = SUPPORTED_NODE_VERSIONS.replace(/[\^~<>=]/g, '')
    .split('||')
    .map((v) => v.trim());
/**
...
function isNodeVersionSupported() {
    ...
    for (const version of exports.supportedNodeVersions) {
        const [major, minor, patch] = version.split('.', 3).map((part) => Number(part));
        if ((major === processMajor && processMinor === minor && processPatch >= patch) ||
...
function isNodeVersionMinSupported() {
    ...
    const [processMajor, processMinor, processPatch] = process.versions.node
        .split('.', 3)
        .map((part) => Number(part));
    const [major, minor, patch] = exports.supportedNodeVersions[0].split('.', 3).map((part) => Number(part));
    return (processMajor > major ||
